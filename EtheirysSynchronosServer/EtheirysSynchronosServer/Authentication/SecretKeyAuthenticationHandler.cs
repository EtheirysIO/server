...
namespace EtheirysSynchronosServer.Authentication
{
    public class SecretKeyAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
    {
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            if (string.IsNullOrEmpty(uid))
            {
                try
                {
                    ...
                    uid = (await _EthDbContext.Auth.Include("User").AsNoTracking()
        ...
        public SecretKeyAuthenticationHandler(IOptionsMonitor<AuthenticationSchemeOptions> options,
            EthDbContext EthDbContext, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock) : base(options, logger, encoder, clock)
        {
            _EthDbContext = EthDbContext;
        }
    }
}
